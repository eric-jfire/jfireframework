...
public class Entry
{
    ...
    public static final int takeFail    = 0;
    public static final int takeSuccess = 1;
    
    public int takeReturnMore()
    {
        if (takeState == FREEFORUSE)
        {
            return unsafe.compareAndSwapInt(this, takeStateoffset, FREEFORUSE, TAKEN) ? takeSuccess : takeFail;
        }
        else
        {
            return ignore;
